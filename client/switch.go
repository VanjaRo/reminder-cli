...
type BackendHTTPClient interface {
	Create(title, msg string, duration time.Duration) ([]byte, error)
	Edit(id, title, msg string, duration time.Duration) ([]byte, error)
...
func (s Switch) edit() func(string) error {
	return func(cmd string) error {
		...
		editCmd.Var(&ids, "id", "The ID (int) of a reminder to edti")
...
func (s Switch) fetch() func(string) error {
	return func(cmd string) error {
		...
		fetchCmd.Var(&ids, "id", "List of reminders IDs (int) to fetch")
...
func (s Switch) delete() func(string) error {
	return func(cmd string) error {
		...
		deleteCmd.Var(&ids, "id", "List of reminders IDs (int) to delete")
...
func (s Switch) checkArgs(minArgs int) error {
	...
	if len(os.Args)-2 < minArgs {
		...
		return fmt.Errorf(
			"%s expects at least %d argument(s), %d provided",
